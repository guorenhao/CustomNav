...
- (void)createItem{
    ...
    if (_menuTitles.count == 1) {
        ...
        RHMenuButtonItem * item = [[RHMenuButtonItem alloc] initWithFrame:CGRectMake(SCREEN_WIDTH/2-ITEM_WIDTH, 0, 2*ITEM_WIDTH, ITEM_HEIGHT) andTitle:_menuTitles[0] andNormalColor:NORMAL_COLOR andSelectColor:SELECT_COLOR];
    ...
    }else if (SCREEN_WIDTH >= ITEM_WIDTH*_menuTitles.count) {
        ...
        for (int i = 0; i < _menuTitles.count; i++) {
            ...
            RHMenuButtonItem * item = [[RHMenuButtonItem alloc] initWithFrame:CGRectMake(((SCREEN_WIDTH-(_menuTitles.count+1)*10)/_menuTitles.count+10)*i+10, 0, (SCREEN_WIDTH-(_menuTitles.count+1)*10)/_menuTitles.count, ITEM_HEIGHT) andTitle:_menuTitles[i] andNormalColor:NORMAL_COLOR andSelectColor:SELECT_COLOR];
...
- (void)itemClick:(RHMenuButtonItem *)item{
    ...
    if (SCREEN_WIDTH >= ITEM_WIDTH*_menuTitles.count) {
    ...
    if (item.tag > 1000 && item.tag - 1000 < _menuTitles.count-1) {
        //使item居中
       [_scrollView setContentOffset:CGPointMake(item.center.x-SCREEN_WIDTH/2, 0) animated:YES];
